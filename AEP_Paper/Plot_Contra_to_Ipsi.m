...
% - loads models for contra/ipsi, left/right
...
for hemi=1:length(Hemi)
    ...
    subplot(1,length(Hemi),hemi_i)
    hold on   
    title(strcat(Hemi{hemi}))
    %plot data
    plot(data_time, data_contra.*-1,'Color',[109 187 228]./255,'Linewidth',4)
    plot(data_time, data_ipsi.*-1,'Color',[226 113 113]./255,'Linewidth',4)
    
    %load and plot contra
    param_name=strcat(Hemi{hemi},'_Contra');
    ...
    %load and plot ipsi
    param_name=strcat(Hemi{hemi},'_Ipsi');
    dpl=load(strcat(modeldata_dir,'\',param_name,'\dpl.txt'));
    sim_time=dpl(:,1);
    dpl_agg=dpl(:,2);%aggregate
...
legend('contra data','ipsi data','contra model', 'ipsi model')        
